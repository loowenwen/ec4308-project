...
for (z_name in names(Z_list)) {
  ...
  for (h in horizons) {
    ...
    out <- foreach(i = 1:nprev, .packages = c("hdm")) %dopar% {
      # parallel work inside horizon
    }
  ...
  for (res in horizon_outputs) {
    h <- res$h
    if (!is.null(res$skip) && res$skip) {
      cat("  Skipping horizon", h, "(already done)\n")
      next
    } else if (!is.null(res$out$error)) {
      cat("  Error at", z_name, "h=", h, ":", res$out$error, "\n")
      next
    }
    results[[z_name]][[paste0("h", h)]] <- res$out
    cat("    Horizon", h, "done.\n")
