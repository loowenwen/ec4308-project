...
```{r lasso plug in F matrices nprev=100, add X for pc0s}
...
plot_forecasts <- function(res_list, y_vec, method_name) {
  df <- lapply(names(res_list), function(z_name) {
    lapply(names(res_list[[z_name]]), function(h) {
  ...
  ggplot(df, aes(x = Time)) +
    ...
    geom_line(aes(y = Actual), color = "black", linetype = "dashed", size = 0.8) +
...
# Example usage
plot_forecasts(res_plugin_lasso, y_t$CPI_t, "Plug-in Lasso")
plot_forecasts(res_bic_lasso, y_t$CPI_t, "BIC Lasso")
