...
runrlasso_Z <- function(Z, y, pl = FALSE) {
  # Combine predictors and dummy if needed
  df <- as.data.frame(Z)
  
  # Fit (post-)LASSO model
  model <- rlasso(df, y, post = pl)
  
  # Predict the next observation (last row of Z)
...
for (z_name in names(Z_list)) {
  ...
  # Tail y to match Z
  if (length(y) > nrow(Z)) {
    y_aligned <- tail(y, nrow(Z))
    cat("  Tailed y from", length(y), "to", length(y_aligned), "\n")
  } else {
    y_aligned <- y
  }
  
  # Quick check: first few dates
  cat("  First few Z dates:", head(Z_full$sasdate), "\n")
  cat("  First few y dates:", head(y_aligned), "\n") # if y has dates, or indices if it's a vector
  
  for (h in horizons) {
    cat("  Horizon:", h, "\n")
    
    # Shift y for horizon h
    y_h <- dplyr::lead(y_aligned, h)
    
    # Run rolling window only where complete data is available
    valid_idx <- 1:(length(y_h) - h)
    
    cat("Length(y_h):", length(y_h), "  Horizon:", h, "\n")
    cat("Valid_idx range:", range(valid_idx), "\n")
    
    out <- rlasso.rolling.window.Z(Z[valid_idx, ], y_h[valid_idx], nprev = nprev, pl = FALSE)
...
# Access example:
# results[["Z1"]][["h3"]]$errors
# results[["Z1"]][["h3"]]$pred

# Loop through results and print RMSE per horizon per Z
