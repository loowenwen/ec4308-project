...
dename_rmse_mae <- function(res_list) { #because some rmse/mae are named num
  ...
  # Otherwise, handle the nested Z → horizon structure
...
# evaluating grc forecasts
grc_eval <- map_dfr(horizon_steps, function(h) {
  
  # Lead the actuals by h steps
  actuals_aligned <- dplyr::lead(y_h_real, h)
  
  # Extract GR predictions for this horizon
  preds <- grc_forecast_df %>%
    filter(horizon == paste0("h", h)) %>%
    pull(grc_pred)
...
```{r unrestricted ensemble}
# store results
grunr_forecasts <- list()
grunr_weights   <- list()
...
for(h in horizon_steps){
  ...
  # Solve for GR weights
  grunr_sol <- lsei(X, y_h)
  
  beta <- grunr_sol   # weights
  
  # Compute GR forecast
  grunr_forecast <- X %*% beta
  
  # Save
  grunr_forecasts[[paste0("h", h)]] <- grunr_forecast
  grunr_weights[[paste0("h", h)]]   <- beta
}

# Combine GR forecasts into a tibble
nprev <- 100  # desired length

grunr_forecast_df <- bind_rows(
  lapply(names(grunr_forecasts), function(h) {
    preds <- grunr_forecasts[[h]]
    # pad with NAs if shorter than nprev
    if(length(preds) < nprev) {
      preds <- c(preds, rep(NA, nprev - length(preds)))
    }
...
# GR weights per horizon
grunr_weights_df <- tibble(
  horizon = names(grunr_weights),
  weights = I(grunr_weights)
)
...
# evaluating gr forecasts
grunr_eval <- map_dfr(horizon_steps, function(h) {
  
  # Lead the actuals by h steps
  actuals_aligned <- dplyr::lead(y_h_real, h)
  
  # Extract GR predictions for this horizon
  preds <- grunr_forecast_df %>%
    filter(horizon == paste0("h", h)) %>%
    pull(grunr_pred)
...
```{r unrestricted with constant ensemble}
# store results
grunrc_forecasts <- list()
grunrc_weights   <- list()
...
for(h in horizon_steps){
  ...
  # Solve for GR weights
  grunrc_sol <- lsei(X, y_h)
  
  beta <- grunrc_sol   # weights
  
  # Compute GR forecast
  grunrc_forecast <- X %*% beta
  
  # Save
  grunrc_forecasts[[paste0("h", h)]] <- grunrc_forecast
  grunrc_weights[[paste0("h", h)]]   <- beta
}

# Combine GR forecasts into a tibble
nprev <- 100  # desired length

grunrc_forecast_df <- bind_rows(
  lapply(names(grunrc_forecasts), function(h) {
    preds <- grunrc_forecasts[[h]]
    # pad with NAs if shorter than nprev
    if(length(preds) < nprev) {
      preds <- c(preds, rep(NA, nprev - length(preds)))
    }
...
# GR weights per horizon
grunrc_weights_df <- tibble(
  horizon = names(grunrc_weights),
  weights = I(grunrc_weights)
)
...
# evaluating gr forecasts
grunrc_eval <- map_dfr(horizon_steps, function(h) {
  
  # Lead the actuals by h steps
  actuals_aligned <- dplyr::lead(y_h_real, h)
  
  # Extract GR predictions for this horizon
  preds <- grunrc_forecast_df %>%
    filter(horizon == paste0("h", h)) %>%
    pull(grunrc_pred)
...
```{r lasso weights ensemble}
# store results
rlassocomb_forecasts <- list()
rlassocomb_weights   <- list()
...
for(h in horizon_steps){
  ...
  # Solve for GR weights (unrestricted ridge-lasso combination)
  rlassocomb_sol <- rlasso(y_h~X, post=FALSE, intercept=FALSE)
  
  beta <- rlassocomb_sol$coefficients   # weights
  
  # Compute GR forecast
  rlassocomb_forecast <- X %*% beta
  
  # Save
  rlassocomb_forecasts[[paste0("h", h)]] <- rlassocomb_forecast
  rlassocomb_weights[[paste0("h", h)]]   <- beta
}

# Combine forecasts into a tibble
nprev <- 100  # desired length

rlassocomb_forecast_df <- bind_rows(
  lapply(names(rlassocomb_forecasts), function(h) {
    preds <- rlassocomb_forecasts[[h]]
    # pad with NAs if shorter than nprev
    if(length(preds) < nprev) {
      preds <- c(preds, rep(NA, nprev - length(preds)))
    }
    tibble(
      horizon = h,
      rlassocomb_pred = preds
    )
  })
)

# Weights per horizon
rlassocomb_weights_df <- tibble(
  horizon = names(rlassocomb_weights),
  weights = I(rlassocomb_weights)
)

rlassocomb_forecast_df
rlassocomb_weights_df

# Evaluate forecasts
rlassocomb_eval <- map_dfr(horizon_steps, function(h) {
  
  # Lead the actuals by h steps
  actuals_aligned <- dplyr::lead(y_h_real, h)
  
  # Extract predictions for this horizon
  preds <- rlassocomb_forecast_df %>%
    filter(horizon == paste0("h", h)) %>%
    pull(rlassocomb_pred)
...
```
```{r lasso weights with constant ensemble}
# store results
rlassocombc_forecasts <- list()
rlassocombc_weights   <- list()
...
for(h in horizon_steps){
  ...
  # Solve for GR weights (unrestricted ridge-lasso combination with constant)
  rlassocombc_sol <- rlasso(y_h ~ X, post = FALSE, intercept = TRUE)
  
  beta <- rlassocombc_sol$coefficients   # weights
  
  # Add a column of 1s for constant
  X <- cbind(const = 1, X)
  
  # Compute GR forecast
  rlassocombc_forecast <- X %*% beta
  
  # Save
  rlassocombc_forecasts[[paste0("h", h)]] <- rlassocombc_forecast
  rlassocombc_weights[[paste0("h", h)]]   <- beta
}

# Combine forecasts into a tibble
nprev <- 100  # desired length

rlassocombc_forecast_df <- bind_rows(
  lapply(names(rlassocombc_forecasts), function(h) {
    preds <- rlassocombc_forecasts[[h]]
    # pad with NAs if shorter than nprev
    if(length(preds) < nprev) {
      preds <- c(preds, rep(NA, nprev - length(preds)))
    }
    tibble(
      horizon = h,
      rlassocombc_pred = preds
    )
  })
)

# Weights per horizon
rlassocombc_weights_df <- tibble(
  horizon = names(rlassocombc_weights),
  weights = I(rlassocombc_weights)
)

rlassocombc_forecast_df
rlassocombc_weights_df

# Evaluate forecasts
rlassocombc_eval <- map_dfr(horizon_steps, function(h) {
  
  # Lead the actuals by h steps
  actuals_aligned <- dplyr::lead(y_h_real, h)
  
  # Extract predictions for this horizon
  preds <- rlassocombc_forecast_df %>%
    filter(horizon == paste0("h", h)) %>%
    pull(rlassocombc_pred)
...
# Add GR with constant ensemble results
grc_eval_df <- grc_eval %>%
  mutate(
    model = "grc_ensemble",
    Z = "all",
    horizon = paste0("h", horizon)
...
# Add GR unrestricted ensemble results
grunr_eval_df <- grunr_eval %>%
  mutate(
    model = "grunr_ensemble",
    Z = "all",
    horizon = paste0("h", horizon)
  ) 

# Add GR unrestricted with constant ensemble results
grunrc_eval_df <- grunrc_eval %>%
  mutate(
    model = "grunrc_ensemble",
    Z = "all",
    horizon = paste0("h", horizon)
  ) 

# Add lasso-weights ensemble results
rlassocomb_eval_df <- rlassocomb_eval %>%
  mutate(
    model = "rlassocomb_ensemble",
    Z = "all",
    horizon = paste0("h", horizon)
  ) 

# Add lasso-weights with constant ensemble results
rlassocombc_eval_df <- rlassocombc_eval %>%
  mutate(
    model = "rlassocombc_ensemble",
    Z = "all",
    horizon = paste0("h", horizon)
  ) 
...
combined_results <- bind_rows(best_models_df, simple_avg_df %>%
  ...
  select(model, horizon, Z, rmse = RMSE, mae = MAE), grc_eval_df%>%
  select(model, horizon, Z, rmse = RMSE, mae = MAE), grunr_eval_df%>%
  select(model, horizon, Z, rmse = RMSE, mae = MAE), grunrc_eval_df%>%
  select(model, horizon, Z, rmse = RMSE, mae = MAE), rlassocomb_eval_df%>%
  select(model, horizon, Z, rmse = RMSE, mae = MAE), rlassocombc_eval_df%>%
...
# Add GR unrestricted with constant ensemble
best_models_all <- add_model_to_best_models_all(
  best_models_all,
  "grunrc_ensemble",
  grunrc_eval_df
...
# Add lasso weights with constant ensemble
best_models_all <- add_model_to_best_models_all(
  best_models_all,
  "rlassocombc_ensemble",
  rlassocombc_eval_df
)

# Save
saveRDS(best_models_all, "all_best_models_preds.rds")

rw_model <- list(
  h1 = list(Z = "yt-1", pred = rw1c, rmse = rw.rmse1, mae = NULL),
  h3 = list(Z = "yt-1", pred = rw3c, rmse = rw.rmse3, mae = NULL),
  h6 = list(Z = "yt-1", pred = rw6c, rmse = rw.rmse6, mae = NULL),
  h12 = list(Z = "yt-1", pred = rw12c, rmse = rw.rmse12, mae = NULL)
)

# Add to your tibble
best_models_all$randomwalk <- rw_model

```


```{r plots??}
plot_forecast_horizon <- function(y_real, preds_list, horizon) {
  
  nprev <- length(y_real)
  start_oos <- as.Date("2017-03-01")
  oos_dates <- seq.Date(from = start_oos, by = "month", length.out = nprev)
  
  # Lead actuals by horizon
  actuals_shifted <- dplyr::lead(y_real, horizon)
  
  df_list <- imap(preds_list, function(model_obj, model_name) {
    preds <- model_obj[[paste0("h", horizon)]]$pred
    
    tibble(
      date = oos_dates + months(horizon),
      actual = actuals_shifted,
      forecast = preds,
      model = model_name
    )
  }) %>%
  ...
  num_models <- length(preds_list)
  colors_for_models <- brewer.pal(min(num_models, 8), "Dark2")  # Dark2 has 8 distinguishable colors
  if (num_models > 8) colors_for_models <- rep(colors_for_models, length.out = num_models)
  
  ggplot(df_list, aes(x = date, y = value)) +
    geom_line(
      data = subset(df_list, type == "actual"),
      aes(group = 1),
      color = "black",
      alpha = 0.7,
      linewidth = 0.8
    ...
    geom_line(
      data = subset(df_list, type == "forecast"),
      aes(color = model),
      linewidth = 0.8,
      alpha = 0.7
    ) +
    scale_color_manual(values = colors_for_models) +
    theme_minimal(base_size = 13) +
    labs(
      title = paste0("Forecasts vs Actuals (horizon = ", horizon, ")"),
      x = "Date",
      y = "CPI_t (Inflation)",
      color = "Model"
    ) +
    coord_cartesian(xlim = c(start_oos, max(oos_dates)))
}
...
plot_forecast_horizon(
  y_real = y_h_real,
  preds_list = list(
    ridge = best_models_all$ridge,
    grunrc_ensemble = best_models_all$grunrc_ensemble,
    randomwalk = best_models_all$randomwalk
  ),
  horizon = 1
)
...
plot_forecast_horizon(
  ...
  preds_list = list(
    ridge = best_models_all$ridge,
    bic_lasso = best_models_all$bic_lasso,
    randomwalk = best_models_all$randomwalk
  ),
  horizon = 3
)
...
plot_forecast_horizon(
  ...
  preds_list = list(
    bic_lasso = best_models_all$bic_lasso,
    elasticnet = best_models_all$elasticnet,
    randomwalk = best_models_all$randomwalk
  ),
  horizon = 12
)
