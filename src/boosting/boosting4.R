...
gradient_boosting_best_model <- function(data_matrix_boost, parameters_boost, time_horizon, dataset_name) {
  train_boosting <- data_matrix_boost %>%
    head(nrow(data_matrix_boost) - 70) %>%
    select(-sasdate) %>%
    na.omit() %>%
    as_tibble()
...
predict_test_set_per_time_horizon <- function(data_matrix_boosting, parameters_boost, time_horizon, dataset_name, test_start_date = as.Date("2020-01-01")) {
